...
const PeVentasSOController = require('../../../Methods/CargaArchivos/PeVentasSO/PeVentasSO')
const moment = require('moment');

controller.ValCargarPeVentasSO = async (req, res) => {
    ...
    try{
        ...
        // **************************************************************
        // VALIDAR QUE EXISTE EL ARCHIVO Y QUE LA HOJA SE LLAMA DATA
        // **************************************************************
        ...
        if(action_file.process_data){
            PeVentasSOController.MetCargarPeVentasSO(req, res, data, borrar_data, false)
        }else{
...
controller.ValCellsFile = async (workbook, usu, date) => {
    ...
    for await (const row of rows){
        ...
        if(!row[properties[0]]){
            ...
            id_mcl_grow = cod_dts.findIndex(dts => dts.codigo_destinatario == row[properties[0]]);
        ...
        data.push({
            ...
            m_cl_grow                       : id_mcl_grow.id?id_mcl_grow.id:null,
